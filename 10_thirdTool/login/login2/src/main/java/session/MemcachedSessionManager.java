...
public class MemcachedSessionManager {
    ...
    private void saveSession(MemcachedHttpSession session) {
        try {
            ...
            if(session.expired)
                ...
                memcachedClient.set(generatorSessionKey(session.id),  session);
        } catch (Exception e) {
    ...
    private MemcachedHttpSession loadSession(String sessionId) {
        try {
            ...
            MemcachedHttpSession session = (MemcachedHttpSession) memcachedClient.get(generatorSessionKey(sessionId));
			if (log.isDebugEnabled())
